...
if(@ARGV != 3) {
	die "usage: $0 ods_file pib_file conv_file";
}
...
if(!open(PIB,$pib_file)) {
	die "I can not open conversion file $pib_file\n";
}
...
while(my $line = <PIB>) {
	...
	if($conv_data{$fields[0]}) {
		...
		#$fields[5] =~ s/\r//g;
		$fields[3] =~ s/\.//g;
		$pop_data{$prov}{$fields[2]} = $fields[3]/1000000;
	}
}
...
my $header = <IN>;
chomp $header;
print $header . "\tPROD_NETA(GWH)_X_PIB(G)\n";
while(my $line = <IN>) {
	chomp $line;
	my @fields = split("\t",$line);
	my $prov = $fields[0];
	...
	if($pop_data{$prov} && $pop_data{$prov}{$year}) {
		...
		print $line ;
		print "\t";
		print sprintf("%.2f",$fields[3]/$pop_data{$prov}{$year});
		print "\n";
